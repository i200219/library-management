...
const BookRequestsPage = () => {
  ...
  return (
    <section className="max-w-6xl mx-auto py-12 px-4">
      <h1 className="text-2xl font-bold mb-4 text-blue-900">Book Borrow Requests</h1>
      <p className="text-md text-gray-700 mb-8">
        Here you can view, approve, or decline book borrow requests.
      </p>
      <div className="bg-white rounded-lg shadow p-6">
        {loading ? (
          <div className="text-center py-8 text-gray-400">Loading requests...</div>
        ) : requests.length === 0 ? (
          <div className="text-center py-8 text-gray-400">No book borrow requests found.</div>
        ) : (
            ...
            <tbody>
              {requests.map((req) => (
                <tr key={req.id} className="hover:bg-blue-100">
                  ...
                  <td className="py-2 px-4 border-b">{req.borrowDate ?? "-"}</td>
